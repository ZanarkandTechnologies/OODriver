---
name: oodriver-scenario-operator
description: Use when generating, curating, running, evaluating, replaying, or exporting OODriver out-of-distribution driving scenarios through the DriverX CLI database.
---

# OODriver Scenario Operator

OODriver is the out-of-distribution scenario generator for 0xDriver. This skill
...
```bash
PYTHONPATH=src python3 -m driverx oodriver init \
  --output-root artifacts/runs \
  --run-id oodriver-demo \
  --force
```
...
```bash
PYTHONPATH=src python3 -m driverx oodriver compile \
  --db artifacts/runs/oodriver-demo/scenario_studio_db.json \
  --count 12 \
  --severity 4 \
  --seed 42

PYTHONPATH=src python3 -m driverx oodriver queue \
  --db artifacts/runs/oodriver-demo/scenario_studio_db.json \
  --accept top:3
```
...
Use Alpamayo through `oodriver evaluate`, not by claiming live control.

6. Evaluate, replay, and export.

```bash
PYTHONPATH=src python3 -m driverx oodriver evaluate \
  --db artifacts/runs/oodriver-demo/scenario_studio_db.json \
  --policy alpamayo-trajectory \
  --memory auto

PYTHONPATH=src python3 -m driverx oodriver replay \
  --db artifacts/runs/oodriver-demo/scenario_studio_db.json

PYTHONPATH=src python3 -m driverx oodriver export \
  --db artifacts/runs/oodriver-demo/scenario_studio_db.json
```
