...
| 1:55-2:25 | Long CARLA OOD Video | Play the scripted CARLA OOD video with overlays and show entity risk evidence. | The DriverX scripted CARLA OOD demo produced a 24.0s video for generated-base-animals-0076-regional-driving-behavior-000 with worst risk {'tick': 27, 'actor_ref': 'generated_asset_asset_fallen_cargo_sack', 'distance_m': 0.1967}. |
| 2:25-3:00 | Alpamayo Memory Test | Show Alpamayo no-memory vs memory CoC snippets and trajectory delta. | The live Alpamayo memory comparison used the generated scene: latency 108765.59ms / 46129.12ms, CoC snippet available=True, memory changed trajectory final L2 by 2.6886m. |
| 3:00-3:30 | Next Run | Show blockers.md and the exact next live command. | 2026-05-06 11:48 +0800 \| fail2drive,carla,town13,score,capture \| TASK-060 long-score attempt `town13-long-score-attempt-001` started the stock `Generalization_PedestriansOnRoad_1088` route and reached game time `0.600s` at about `0.142x`, then stopped making observable progress. A concurrent route-aligned Alpamayo capture attempt with a 60s CARLA timeout also failed waiting for the simulator. I terminated the route evaluator cleanly to avoid burning the full 1200s timeout on a stalled local Mac/Wine simulation. Evidence: `tickets/TASK-060/artifacts/town13-long-score-attempt-001-evidence/run_evidence.md` and `tickets/TASK-069/artifacts/town13-live-attach-attempt-004/carla_alpamayo_capture.json`. Next unblock path: use a graphics-capable Linux NVIDIA CARLA host for Fail2Drive scoring/capture, or rerun locally only after confirming CARLA can sustain route ticks and serve a second Python client during synchronous mode. |
...
- `alpamayo_comparison_path`: `tickets/TASK-081/artifacts/task81-live-same-scene-comparison/alpamayo_ood_comparison.json`
- `ood_video_evidence_path`: `tickets/TASK-079/artifacts/task79-live-ood-video/ood_video_evidence.json`
- `alpamayo_scene_path`: `tickets/TASK-080/artifacts/task80-live-same-scene-alpamayo-scene-v2/alpamayo_ood_scene.json`
- `generated_asset_evidence_path`: `tickets/TASK-076/artifacts/stock-proxy-assets-v2/asset_summary.json`
...
- trajectory_delta: `{'available': True, 'point_count': 20, 'mean_l2_m': 1.0226, 'max_l2_m': 2.6886, 'final_l2_m': 2.6886}`
...
- `alpamayo-live-ood-comparison`: nvidia/Alpamayo-1.5-10B (same_capture_memory_comparison_ready)
- `alpamayo-generated-ood-scene`: nvidia/Alpamayo-1.5-10B (ready)
...
- Current scripted CARLA OOD video evidence: tickets/TASK-079/artifacts/task79-live-ood-video/generated-base-animals-0076-regional-driving-behavior-000_ood.mp4 (24.0s)
- Current generated object evidence uses stock CARLA proxy assets; artifact: tickets/TASK-076/artifacts/stock-proxy-assets-v2/asset_manifests.json
...
The local harness is reproducible: local demo status is `ready`, behavior is `motorcycle_filtering`, policy runtime ready rows are `3`, and the Alpamayo probe status is `dataset_shape_observed`. Scripted CARLA OOD video duration is `24.0` seconds. Alpamayo scene reasoning latency is `108765.59` ms. The live Alpamayo memory comparison changed trajectory final L2 by `2.6886` metres while staying explicitly open-loop. Cached replay produced `20` bounded control command(s) from a saved policy decision without claiming real-time VLA control.
