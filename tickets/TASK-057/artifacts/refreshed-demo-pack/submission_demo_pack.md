...
| 2:10-2:45 | Alpamayo Memory Test | Show Alpamayo no-memory vs memory CoC snippets and trajectory delta. | Alpamayo is now a live open-loop policy probe: status model_loaded, memory changed trajectory final L2 by 2.8886m. |
| 2:45-3:20 | Next Run | Show blockers.md and the exact next live command. | 2026-05-06 01:50 +0800 \| fail2drive,carla,map \| TASK-054B proved the Docker Fail2Drive client can reach local CARLA, but the stock Fail2Drive split routes in `fail2drive_split/` all require `Town13`, which is not installed in the local CARLA 0.9.16 package. Evidence: `tickets/TASK-054B/artifacts/docker-route-run-town13-map-blocker-classified/fail2drive_route_run.md`. Next unblock path: install/provide a CARLA package containing Town13 or use a compatible route generator for installed maps such as Town10HD_Opt. |
...
- `policy_matrix_path`: `artifacts/runs/task37-policy-matrix-review/policy_runtime_matrix.json`
- `alpamayo_probe_path`: `tickets/TASK-052/artifacts/probe-load-eager-after-cosmos-summary/alpamayo_probe_report.json`
- `route_evidence_path`: `tickets/TASK-055/artifacts/town10-route-evidence/run_evidence.json`
- `alpamayo_comparison_path`: `tickets/TASK-056/artifacts/town10-memory-comparison/alpamayo_ood_comparison.json`
...
- route_video: `tickets/TASK-054B/artifacts/docker-route-plan-town10-speedlimit/fail2drive_outputs/routes_town10.mp4`
...
- `alpamayo-probe`: nvidia/Alpamayo-1.5-10B (model_loaded)
...
- Current local route video evidence: tickets/TASK-054B/artifacts/docker-route-plan-town10-speedlimit/fail2drive_outputs/routes_town10.mp4 (4.1s, 7886364 bytes)
...
The local harness is reproducible: generated suite status is `blocked`, policy runtime ready rows are `5`, and the Alpamayo probe status is `model_loaded`. The live Alpamayo memory comparison changed trajectory final L2 by `2.8886` metres while staying explicitly open-loop.
