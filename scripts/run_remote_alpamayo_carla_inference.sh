...
ssh ${SSH_OPTIONS} "$REMOTE" "PATH=\"\$HOME/.local/bin:\$PATH\" REMOTE_ROOT='$REMOTE_ROOT' MODEL_ID='$MODEL_ID' ALPAMAYO_NUM_TRAJ_SAMPLES='$ALPAMAYO_NUM_TRAJ_SAMPLES' ALPAMAYO_MAX_GENERATION_LENGTH='$ALPAMAYO_MAX_GENERATION_LENGTH' ALPAMAYO_ATTN_IMPLEMENTATION='$ALPAMAYO_ATTN_IMPLEMENTATION' PYTHON_BIN='$PYTHON_BIN' UV_CACHE_DIR='$REMOTE_CACHE_ROOT/uv' TRANSFORMERS_CACHE='$REMOTE_CACHE_ROOT/huggingface' HF_HUB_CACHE='$REMOTE_CACHE_ROOT/huggingface/hub' '$PYTHON_BIN' '$REMOTE_ROOT/carla_inference.py'; status=\$?; rm -f '$REMOTE_ROOT/.hf_token'; exit \$status"
