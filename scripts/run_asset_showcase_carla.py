"""Render asset-grounded OODrive showcase videos on a live CARLA server."""
...
CASES = (
    AssetShowcaseCase(
        case_id="asset_farm_haybale_lane_block",
        title="Farm haybale lane block",
        paragraph=(
            "Generate a rural road where haybales have fallen from a farm trailer and "
            "partially block the lane. The ego car should drive at least 60 meters so "
            "the video shows the generated haybale assets in the road context. "
            "The camera should make the placed assets clearly visible, not just prove them in JSON."
        ),
        weather={
            "cloudiness": 35,
            "precipitation": 0,
        ...
        route_distance_m=105,
        target_drive_m=65,
        props=(
            {"blueprint": "static.prop.haybale", "s": 14, "lateral": 0.8, "yaw": 25},
            {"blueprint": "static.prop.haybale", "s": 20, "lateral": -0.6, "yaw": -20},
            {"blueprint": "static.prop.trafficwarning", "s": 10, "lateral": 2.5, "yaw": 0},
        ),
        vehicles=({"blueprint": "vehicle.carlamotors.carlacola", "s": 30, "lateral": 4.6, "yaw": 8},),
    ),
    AssetShowcaseCase(
        case_id="asset_market_foodcart_debris_squeeze",
        title="Food-cart debris lane squeeze",
        paragraph=(
            "Generate a wet roadside market hazard where a food cart, cones, and debris "
            "squeeze the lane. The ego car should continue through the asset-rich scene "
            "so the prompt-to-CARLA object placement is visible from the ego vehicle."
        ),
        weather={
            "cloudiness": 85,
            "precipitation": 35,
            "precipitation_deposits": 65,
            "wetness": 85,
            "fog_density": 8,
            "sun_altitude_angle": -5,
            "sun_azimuth_angle": 25,
        },
        route_distance_m=105,
        target_drive_m=65,
        props=(
            {"blueprint": "static.prop.foodcart", "s": 13, "lateral": 3.3, "yaw": 95},
            {"blueprint": "static.prop.constructioncone", "s": 18, "lateral": 1.3, "yaw": 0},
            {"blueprint": "static.prop.constructioncone", "s": 23, "lateral": 1.2, "yaw": 0},
            {"blueprint": "static.prop.dirtdebris01", "s": 31, "lateral": 1.1, "yaw": 25},
        ),
        vehicles=({"blueprint": "vehicle.vespa.zx125", "s": 38, "lateral": -3.0, "yaw": 180},),
    ),
    AssetShowcaseCase(
        case_id="asset_crash_barrier_debris_chicane",
        title="Crash barrier debris chicane",
        paragraph=(
            "Generate a post-accident chicane with street barriers, cones, debris, and "
            "stopped vehicles. The ego car should drive roughly 80 meters while the "
            "video makes the obstacles visible instead of showing an empty road."
        ),
        weather={
            "cloudiness": 65,
            "precipitation": 8,
        ...
        route_distance_m=125,
        target_drive_m=80,
        props=(
            {"blueprint": "static.prop.streetbarrier", "s": 15, "lateral": 1.2, "yaw": 12},
            {"blueprint": "static.prop.constructioncone", "s": 23, "lateral": -1.4, "yaw": 0},
            {"blueprint": "static.prop.dirtdebris02", "s": 31, "lateral": 1.0, "yaw": 40},
            {"blueprint": "static.prop.streetbarrier", "s": 39, "lateral": -1.5, "yaw": -18},
        ),
        vehicles=(
            {"blueprint": "vehicle.tesla.model3", "s": 28, "lateral": 4.7, "yaw": 15},
            {"blueprint": "vehicle.dodge.charger_police_2020", "s": 50, "lateral": -4.6, "yaw": 190},
        ),
    ),
)
...
def main() -> None:
    ...
    output_root = repo / "artifacts" / "runs" / "task188-asset-showcase-live-v2"
    output_root.mkdir(parents=True, exist_ok=True)
    client = carla.Client("127.0.0.1", 2000)
    client.set_timeout(45)
    world = client.get_world()
    route = _choose_route(world.get_map(), min_distance=max(case.route_distance_m for case in CASES))
    manifest = {
        "schema_version": "oodrive.asset_showcase_carla.v1",
        "map": world.get_map().name,
        "cases": [],
        "claim_boundaries": [
            "prompt_to_carla_assets=true",
            "stock_and_fail2drive_compatible_blueprints=true",
            "generated_custom_meshes=false",
            "closed_loop_vla_control=false",
            "animal_assets=false_on_current_stock_carla_server",
        ],
    }
    for case in CASES:
        manifest["cases"].append(_render_case(world, route, case, output_root))
    manifest_path = output_root / "asset_showcase_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print(json.dumps({**manifest, "manifest_path": str(manifest_path)}, indent=2))


def _render_case(world: carla.World, route: list[carla.Waypoint], case: AssetShowcaseCase, output_root: Path) -> dict[str, Any]:
    case_dir = output_root / case.case_id
    frames_dir = case_dir / "frames"
    frames_dir.mkdir(parents=True, exist_ok=True)
    (case_dir / "prompt.md").write_text(f"# {case.title}\n\n{case.paragraph}\n", encoding="utf-8")
    cleanup: list[carla.Actor] = []
    spawned_assets: list[dict[str, Any]] = []
    ...
    try:
        ...
        cleanup.append(ego)
        for item in [*case.props, *case.vehicles]:
            actor = _spawn_relative(world, route, item)
            spawned_assets.append(
                {
                    "requested_blueprint": item["blueprint"],
        ...
        camera = world.spawn_actor(
            camera_bp,
            carla.Transform(carla.Location(x=1.0, z=2.15), carla.Rotation(pitch=-8)),
            attach_to=ego,
        )
        ...
        scene_spec_path.write_text(json.dumps(_case_spec(route, case, spawned_assets), indent=2), encoding="utf-8")
        return {
            ...
            "spawned_asset_count": sum(1 for asset in spawned_assets if asset["spawned"]),
        }
    finally:
...
def _choose_route(carla_map: carla.Map, *, min_distance: float) -> list[carla.Waypoint]:
    ...
    for spawn in carla_map.get_spawn_points():
        ...
        current = waypoint
        yaw_change = 0.0
        for _ in range(int(min_distance / 3.0) + 20):
...
def _spawn_relative(world: carla.World, route: list[carla.Waypoint], item: dict[str, Any]) -> carla.Actor | None:
    ...
    if bp.has_attribute("role_name"):
        bp.set_attribute("role_name", "oodrive_asset_showcase")
    return world.try_spawn_actor(bp, transform)


def _control_to_target(ego: carla.Vehicle, target: carla.Location, tick: int, target_drive_m: float, start: carla.Location) -> carla.VehicleControl:
    loc = ego.get_location()
    ...
    if driven >= target_drive_m:
        ...
        control.throttle = 0.58 if tick < 130 else 0.46
    return control
...
def _case_spec(route: list[carla.Waypoint], case: AssetShowcaseCase, spawned_assets: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "title": case.title,
        ...
        "target_drive_m": case.target_drive_m,
        "requested_props": case.props,
        "requested_vehicles": case.vehicles,
        "spawned_assets": spawned_assets,
    }
